...
void ResUpdateEx::runUpdate()
{
    CCLOG(" ResUpdateEx::runUpdate ");
    LuaEngine::getInstance()->executeScriptFile("src/public/resUpdate/UpdateEntry.lua");
}
...
void ResUpdateEx::startGame()
{
    ...
    this, 0.0f, 0, 0.3f, false, "ResUpdateEx");
}
...
void ResUpdateEx::onError(const network::Downloader::Error &error)
{
    ...
    pStack->executeFunctionByHandler(_luaFunc_error, 0);
    pStack->clean();
    /*
    LuaEngine::getInstance()->removeScriptHandler(_luaFunc_error);
    LuaEngine::getInstance()->removeScriptHandler(_luaFunc_progress);
