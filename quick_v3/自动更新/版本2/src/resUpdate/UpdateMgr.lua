...
function downloadVersion()
  ...
  local url = getUrl(local_manifest, version_name, "@version")
...
function downloadManifest()
  ...
  local url = getUrl(local_manifest, manifest_name..".zip", "@manifest")
...
function downloadAssets()
  ...
  if #downLoad_list.assets > 0 then 
    ...
    local url = getUrl(remote_manifest, pathname, "@assets")
...
function getUrl(manifest, filePath, customId)

  --下载version文件和 md5 列表文件共用同一个域名, 下载资源使用另一域名
  local host, url 
  local packageUrl = isIOS and manifest.packageUrl_2 or manifest.packageUrl_1 
  if customId == "@version" or customId == "@manifest" then 
    host = packageUrl.checkUrl 

  elseif customId == "@assets" then 
    if retryCount > RETRY_MAX and packageUrl.cdnUrl2 ~= "" then --使用备份下载点
