...
bool AppDelegate::applicationDidFinishLaunching()
{
	...
	std::string externalAssetsPath = cToolsForLua::getExternalAssetsPath();
	if (externalAssetsPath.size() > 0)//只有google包才会有这个路径,也只有google包这个判断才会为真
	{
		FileUtils::getInstance()->addSearchPath(externalAssetsPath + "res/");
		FileUtils::getInstance()->addSearchPath(externalAssetsPath + "res/cocos/");
	}
