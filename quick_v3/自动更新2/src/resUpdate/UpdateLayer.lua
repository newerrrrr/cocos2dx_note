...
local callback_yes, callback_no 

function create(target, storagePath)
  UpdateMgr:init(storagePath) --设置路径, 保证UI资源正常加载
...
function onEnter()
  release_print("@@@ UpdateLayer:onEnter")
  initUI()
...
function onExit()
  release_print("@@@ UpdateLayer:onExit")
  UpdateMgr:exit()
end 
...
function initUI()
  if nil == uiLayer then 
    ...
    if uiLayer then 
      ...
      btn_yes:addClickEventListener(function()
          if callback_yes then 
            callback_yes()
          end 
        end)
      btn_no:addClickEventListener(function()
          if callback_no then 
            callback_no()
          end 
        end)
      node2:getChildByName("Text_3"):setString(lang["msgBox_ok"])
      node2:getChildByName("Text_5"):setString(lang["msgBox_cancle"])
    end 
