...
- (void)initRecordView{
    if (recorderView == nil)
    {
//        recorderView =[[ChatRecorderView alloc] initWithFrame:CGRectMake((self.view.frame.size.width-160)/2, (self.view.frame.size.height-160)/2, 160, 160)];
    recorderView = (ChatRecorderView*)[[[[NSBundle mainBundle]loadNibNamed:@"ChatRecorderView" owner:self options:nil]lastObject]retain];
      
    }
    
//        recorderView = (ChatRecorderView*)[[[[NSBundle mainBundle]loadNibNamed:nil owner:self options:nil]lastObject]retain];
    //还原界面显示
...
- (void)touchEnded:(CGPoint)_point{
    //判断是否移动到取消区域
    canNotSend =CGRectContainsPoint(recorderView.frame,_point)? YES : NO;
    //设置取消动画
    [recorderView prepareToDelete:canNotSend];
    ...
    [UIView hideViewByCompletion:^(BOOL finish){
        ...
        if (canNotSend) {
            ...
            //录音取消回调
            if ([self.vrbDelegate respondsToSelector:@selector(VoiceRecorderBaseVCRecordFail:)])
                [self.vrbDelegate VoiceRecorderBaseVCRecordFail:@"取消发送，删除源文件"];
        }else{
            //回调录音文件路径
